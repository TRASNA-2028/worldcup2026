...
function computeStandings(groupKey) {
  ...
  const scores = (state.groups[groupKey] && state.groups[groupKey].scores) ? state.groups[groupKey].scores : {};
...
function getKOWinner(round, matchIdx) {
  var roundStateObj = state[round];
  var roundState = roundStateObj ? roundStateObj[matchIdx] : null;
  ...
  if (round === 'r32') {
    ...
    if (round === 'third') {
      ...
      const pairingsForRound = pairings[round];
      const pair = pairingsForRound ? pairingsForRound[matchIdx] : null;
      if (!pair) {
        // Final
...
function getKOLoser(round, matchIdx) {
  var roundStateObj2 = state[round];
  var roundState = roundStateObj2 ? roundStateObj2[matchIdx] : null;
  if (!roundState) return { name: 'TBD', flag: '' };

  const prevRound = { r16: 'r32', qf: 'r16', sf: 'qf' }[round];
  const pairings = { r16: WC2026.r16Pairings, qf: WC2026.qfPairings, sf: WC2026.sfPairings };
  const pairingsForRound2 = pairings[round];
  const pair = pairingsForRound2 ? pairingsForRound2[matchIdx] : null;
